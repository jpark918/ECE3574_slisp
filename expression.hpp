...
struct Expression{
  Atom head;
  std::vector<Expression> tail; //emplace back method to push into vector.  //atom can be placed inside vector of expressions

  Expression() {
    head.type = NoneType;
    //----- added
    head.value.bool_value = Boolean();
    head.value.num_value = Number();
    head.value.sym_value = Symbol();
  };
  ...
  //ADDED METHODS HERE
  //bool add(const std::vector<Atom>& args);
  //bool not(const std::vector<Atom>& args);
};
